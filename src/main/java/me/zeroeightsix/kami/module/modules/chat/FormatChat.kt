...
class FormatChat : Module() {
    public override fun onEnable() {
        if (Minecraft.getMinecraft().getCurrentServerData() == null) {
            MessageSendHelper.sendWarningMessage("&9[Silon] &rThis does not work in singleplayer!")
            disable()
        } else {
            MessageSendHelper.sendWarningMessage("&9[Silon] &rThis will kick you on most servers!")
        }
    }
