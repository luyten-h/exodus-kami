...
@Module.Info(
        name = "812e3hiy2tq4fgwejaetf",
        description = "You have found this mysterious module! Do not enable",
...
public class CrystalAura extends Module {
    ...
    public void onUpdate() {
        ...
        if (crystal != null && CrystalAura.mc.player.getDistance((Entity) crystal) <= this.breakRange.getValue()) {
        ...
        for (final Entity entity2 : entities) {
            if (entity2 != CrystalAura.mc.player) {
                ...
                if (CrystalAura.mc.player.getDistanceSq(entity2) > this.enemyRange.getValue() * this.enemyRange.getValue()) {
                    continue;
                }
                for (final BlockPos blockPos : blocks) {
                    if (!canBlockBeSeen(blockPos) && CrystalAura.mc.player.getDistanceSq(blockPos) > 25.0 && this.raytrace.getValue()) {
                        continue;
                    }
        ...
        if (this.place.getValue()) {
            ...
            this.lookAtPacket(finalPos.x + 0.5, finalPos.y - 0.5, finalPos.z + 0.5, (EntityPlayer) CrystalAura.mc.player);
            final RayTraceResult result = CrystalAura.mc.world.rayTraceBlocks(new Vec3d(CrystalAura.mc.player.posX, CrystalAura.mc.player.posY + CrystalAura.mc.player.getEyeHeight(), CrystalAura.mc.player.posZ), new Vec3d(finalPos.x + 0.5, finalPos.y - 0.5, finalPos.z + 0.5));
            EnumFacing f;
            ...
            if (System.nanoTime() / 1000000L - this.placeSystemTime >= this.placeDelay.getValue() * 2) {
                CrystalAura.mc.player.connection.sendPacket((Packet) new CPacketPlayerTryUseItemOnBlock(finalPos, f, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                ++this.placements;
    ...
    private List<BlockPos> findCrystalBlocks() {
        ...
        positions.addAll((Collection)this.getSphere(CrystalAura.getPlayerPos(), this.placeRange.getValue().floatValue(), this.placeRange.getValue(), false, true, 0).stream().filter(this::canPlaceCrystal).collect(Collectors.toList()));
    ...
    public static boolean canBlockBeSeen(final BlockPos blockPos) {
        return CrystalAura.mc.world.rayTraceBlocks(new Vec3d(CrystalAura.mc.player.posX, CrystalAura.mc.player.posY + CrystalAura.mc.player.getEyeHeight(), CrystalAura.mc.player.posZ), new Vec3d((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()), false, true, false) == null;
    }
    ...
    protected void onEnable() {
        ...
        if (this.alert.getValue() && CrystalAura.mc.world != null) {
