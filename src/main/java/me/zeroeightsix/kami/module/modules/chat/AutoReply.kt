...
class AutoReply : Module() {
    ...
    @JvmField
    var message: Setting<String> = register(Settings.stringBuilder("Custom Text").withValue("&rUse &9" + Command.getCommandPrefix() + "autoreply&r to modify this").withConsumer { _: String?, _: String? -> }.withVisibility { customMessage.value }.build())
    ...
    private val receiveListener = Listener(EventHook { event: PacketEvent.Receive ->
        ...
        if (event.packet is SPacketChat && MessageDetectionHelper.isDirect(true, (event.packet as SPacketChat).getChatComponent().unformattedText)) {
            if (customMessage.value) {
                ...
                MessageSendHelper.sendServerMessage("/r I currently don't want to talk to you! [Exodus.exe]")
            }
        }
    })
}