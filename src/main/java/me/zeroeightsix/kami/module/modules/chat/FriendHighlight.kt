...
class FriendHighlight : Module() {
    ...
    public override fun onEnable() {
        if (Friends.friends.value.size > 100) {
            MessageSendHelper.sendErrorMessage("&rYour friends list is bigger then 100, disabling as it would cause too much of a performance impact!")
            disable()
        }
        noFriendsCheck()
    }
    ...
    private fun noFriendsCheck(): Boolean {
        if (Friends.friends.value.size == 0) {
            MessageSendHelper.sendErrorMessage("&rYou don't have any friends added, silly! Go add some friends before using the module.")
            disable()
            return true
        }
        return false
