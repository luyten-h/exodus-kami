...
@Module.Info(
        name = "DispenserMeta",
        description = "Just in case, don't use with any AntiGhostBlock mod",
    ...
    protected void onEnable() {
        ...
        if (obiSlot == -1 || dispenserSlot == -1 || shulkerSlot == -1 || redstoneSlot == -1 || hopperSlot == -1) {
            if (debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("Items missing, disabling.");
            }
            this.disable();
            return;
        }
        ...
        if (mc.objectMouseOver == null || mc.objectMouseOver.getBlockPos() == null || mc.objectMouseOver.getBlockPos().up() == null) {
            if (debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("Not a valid place target, disabling.");
            }
            this.disable();
            return;
        }
        ...
        if (autoEnableBypass.getValue()) {
            OldModuleManager.getModuleByName("IllegalItemBypass").enable();
        }

        if (debugMessages.getValue()) {
            MessageSendHelper.sendChatMessage("Place Target: " + placeTarget.x + " " + placeTarget.y + " " + placeTarget.z + " Distance: " + df.format(mc.player.getPositionVector().distanceTo(new Vec3d(placeTarget))));
        }
    ...
    public void onUpdate() {
        ...
        if (stage == 3) {
            ...
            if (autoEnableHitAura.getValue()) {
                OldModuleManager.getModuleByName("KillAuraX").enable();
            }
