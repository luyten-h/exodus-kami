...
class AutoLog : Module() {
    ...
    fun getItems(i: Item): Int {
        return mc.player.inventory.mainInventory.stream().filter { itemStack: ItemStack -> itemStack.getItem() === i }.mapToInt { obj: ItemStack -> obj.count }.sum() + mc.player.inventory.offHandInventory.stream().filter { itemStack: ItemStack -> itemStack.getItem() === i }.mapToInt { obj: ItemStack -> obj.count }.sum()
    }
