...
class TotemPopCounter : Module() {
    ...
    private val colourCode = register(Settings.e<ColourCode>("Color Name", ColourCode.WHITE))
    private val colourCode1 = register(Settings.e<ColourCode>("Color Number", ColourCode.WHITE))
    ...
    override fun onUpdate() {
        if (!isDead
                ...
                && 0 >= mc.player.health) {
            sendMessage("&4[Exodus]&r " + formatName(mc.player.name) + " died and " + grammar(mc.player.name) + " pop list was reset!")
            isDead = true
            playerList!!.clear()
            return
        }
        ...
        for (player in mc.world.playerEntities) {
            if (resetDeaths.value
                ...
                sendMessage("&4[Exodus]&r " + formatName(player.name) + " died after popping " + formatNumber(playerList!![player.name]!!) + " totems" + ending())
    ...
    private val listListener = Listener(EventHook { event: EntityUseTotemEvent ->
        ...
        if (playerList!![event.entity.name] == null) {
            ...
            sendMessage("&4[Exodus]&r " + formatName(event.entity.name) + " popped " + formatNumber(1) + " totem" + ending())
            ...
            sendMessage("&4[Exodus]&r " + formatName(event.entity.name) + " popped " + formatNumber(popCounter) + " totems" + ending())
        }
    })
