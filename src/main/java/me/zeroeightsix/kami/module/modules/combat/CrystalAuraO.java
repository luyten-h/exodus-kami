...
@Module.Info(
        name = "AutoCrystalO",
        description = "Another CrystalAura",
        category = Module.Category.COMBAT
)
public class CrystalAuraO extends Module {
    ...
    private Setting<Integer> Red = register(
            Settings.integerBuilder("Red").withMinimum(1).withMaximum(255).withValue(165));
    ...
    private Setting<Integer> Blue = register(
            Settings.integerBuilder("Blue").withMinimum(1).withMaximum(255).withValue(0));
    ...
    public CrystalAuraO() {
        ...
        this.packetListener = new Listener<PacketEvent.Send>(event -> {
            ...
            if (packet[0] instanceof CPacketPlayer && CrystalAuraO.isSpoofingAngles) {
                ((CPacketPlayer) packet[0]).yaw = (float) CrystalAuraO.yaw;
                ((CPacketPlayer) packet[0]).pitch = (float) CrystalAuraO.pitch;
            }
    ...
    public void onUpdate() {
        final EntityEnderCrystal crystal = (EntityEnderCrystal) CrystalAuraO.mc.world.loadedEntityList.stream().filter(entity -> entity instanceof EntityEnderCrystal).map(entity -> entity).min(Comparator.comparing(c -> CrystalAuraO.mc.player.getDistance(c))).orElse(null);
        if (crystal != null && CrystalAuraO.mc.player.getDistance((Entity) crystal) <= this.breakRange.getValue()) {
            if (System.nanoTime() / 1000000L - this.breakSystemTime >= 420 - this.attackSpeed.getValue() * 20) {
                this.lookAtPacket(crystal.posX, crystal.posY, crystal.posZ, (EntityPlayer) CrystalAuraO.mc.player);
                CrystalAuraO.mc.playerController.attackEntity((EntityPlayer) CrystalAuraO.mc.player, (Entity) crystal);
                CrystalAuraO.mc.player.swingArm(EnumHand.MAIN_HAND);
        ...
        int crystalSlot = (CrystalAuraO.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) ? CrystalAuraO.mc.player.inventory.currentItem : -1;
        if (crystalSlot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (CrystalAuraO.mc.player.inventory.getStackInSlot(l).getItem() == Items.END_CRYSTAL) {
                    crystalSlot = l;
                    break;
                }
            }
        ...
        entities.addAll((Collection<? extends Entity>) CrystalAuraO.mc.world.playerEntities.stream().filter(entityPlayer -> !Friends.isFriend(entityPlayer.getName())).collect(Collectors.toList()));
        double damage = 0.5;
        for (final Entity entity2 : entities) {
            if (entity2 != CrystalAuraO.mc.player) {
                if (((EntityLivingBase) entity2).getHealth() <= 0.0f) {
                    continue;

                }
                if (CrystalAuraO.mc.player.getDistanceSq(entity2) > this.enemyRange.getValue() * this.enemyRange.getValue()) {
                    continue;
                }
                for (final BlockPos blockPos : blocks) {
                    if (!canBlockBeSeen(blockPos) && CrystalAuraO.mc.player.getDistanceSq(blockPos) > 25.0 && this.raytrace.getValue()) {
                        continue;
                    }
                    ...
                    final double self = calculateDamage(blockPos.x + 0.5, blockPos.y + 1, blockPos.z + 0.5, (Entity) CrystalAuraO.mc.player);
                    if (this.antiSui.getValue()) {
                        if (CrystalAuraO.mc.player.getHealth() + CrystalAuraO.mc.player.getAbsorptionAmount() - self <= 7.0) {
                            continue;
                        }
                        if (self > d) {
                            continue;
        ...
        if (this.place.getValue()) {
            if (!offhand && CrystalAuraO.mc.player.inventory.currentItem != crystalSlot) {
                if (this.autoSwitch.getValue()) {
                    CrystalAuraO.mc.player.inventory.currentItem = crystalSlot;
                    resetRotation();
                    this.switchCooldown = true;
                }
                return;
            }
            this.lookAtPacket(finalPos.x + 0.5, finalPos.y - 0.5, finalPos.z + 0.5, (EntityPlayer) CrystalAuraO.mc.player);
            final RayTraceResult result = CrystalAuraO.mc.world.rayTraceBlocks(new Vec3d(CrystalAuraO.mc.player.posX, CrystalAuraO.mc.player.posY + CrystalAuraO.mc.player.getEyeHeight(), CrystalAuraO.mc.player.posZ), new Vec3d(finalPos.x + 0.5, finalPos.y - 0.5, finalPos.z + 0.5));
            EnumFacing f;
            ...
            if (System.nanoTime() / 1000000L - this.placeSystemTime >= this.placeDelay.getValue() * 2) {
                CrystalAuraO.mc.player.connection.sendPacket((Packet) new CPacketPlayerTryUseItemOnBlock(finalPos, f, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                ++this.placements;
                this.antiStuckSystemTime = System.nanoTime() / 1000000L;
                this.placeSystemTime = System.nanoTime() / 1000000L;
            }
        }
        if (CrystalAuraO.isSpoofingAngles) {
            if (CrystalAuraO.togglePitch) {
                final EntityPlayerSP player = CrystalAuraO.mc.player;
                player.rotationPitch += (float) 4.0E-4;
                CrystalAuraO.togglePitch = false;
            } else {
                final EntityPlayerSP player2 = CrystalAuraO.mc.player;
                player2.rotationPitch -= (float) 4.0E-4;
                CrystalAuraO.togglePitch = true;
            }
        }
    }
    ...
    private boolean canPlaceCrystal(final BlockPos blockPos) {
        ...
        return (CrystalAuraO.mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || CrystalAuraO.mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && CrystalAuraO.mc.world.getBlockState(boost).getBlock() == Blocks.AIR && CrystalAuraO.mc.world.getBlockState(boost2).getBlock() == Blocks.AIR && CrystalAuraO.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost)).isEmpty() && CrystalAuraO.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2)).isEmpty();
    }

    public static BlockPos getPlayerPos() {
        return new BlockPos(Math.floor(CrystalAuraO.mc.player.posX), Math.floor(CrystalAuraO.mc.player.posY), Math.floor(CrystalAuraO.mc.player.posZ));
    }

    private List<BlockPos> findCrystalBlocks() {
        NonNullList positions = NonNullList.create();
        positions.addAll((Collection)this.getSphere(CrystalAuraO.getPlayerPos(), this.placeRange.getValue().floatValue(), this.placeRange.getValue(), false, true, 0).stream().filter(this::canPlaceCrystal).collect(Collectors.toList()));
    ...
    public static float calculateDamage(final double posX, final double posY, final double posZ, final Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion((World) CrystalAuraO.mc.world, (Entity)null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    private static float getDamageMultiplied(final float damage) {
        final int diff = CrystalAuraO.mc.world.getDifficulty().getId();
    ...
    public static boolean canBlockBeSeen(final BlockPos blockPos) {
        return CrystalAuraO.mc.world.rayTraceBlocks(new Vec3d(CrystalAuraO.mc.player.posX, CrystalAuraO.mc.player.posY + CrystalAuraO.mc.player.getEyeHeight(), CrystalAuraO.mc.player.posZ), new Vec3d((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ()), false, true, false) == null;
    }

    private static void setYawAndPitch(final float yaw1, final float pitch1) {
        CrystalAuraO.yaw = yaw1;
        CrystalAuraO.pitch = pitch1;
        CrystalAuraO.isSpoofingAngles = true;
    }

    private static void resetRotation() {
        if (CrystalAuraO.isSpoofingAngles) {
            CrystalAuraO.yaw = CrystalAuraO.mc.player.rotationYaw;
            CrystalAuraO.pitch = CrystalAuraO.mc.player.rotationPitch;
            CrystalAuraO.isSpoofingAngles = false;
        }
    }
    ...
    protected void onEnable() {
        ...
        if (this.alert.getValue() && CrystalAuraO.mc.world != null) {
