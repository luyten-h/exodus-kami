...
class VisualRange : Module() {
    ...
    private val message = register(Settings.b("Message", false))
        ...
        if (tickPlayerList.size > 0) {
            for (playerName in tickPlayerList) {
                ...
                if (!knownPlayers!!.contains(playerName)) {
                    ...
                    if (message.value) MessageSendHelper.sendServerMessage("/w $playerName Watch out, you are in my render distance!")
                    return
                }
            }
        }
            ...
            for (playerName in knownPlayers!!) {
                if (!tickPlayerList.contains(playerName)) {
                    ...
                    if (leaving.value) {
                        ...
                        if (message.value) MessageSendHelper.sendServerMessage("/w $playerName Damn, you are no longer in my render distance!")
                    }
                    return
                }
            }
