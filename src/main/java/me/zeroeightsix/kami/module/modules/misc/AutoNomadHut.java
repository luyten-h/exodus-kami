...
@Module.Info(
        name = "AutoNomadHut",
        description = "Builds a Nomad Hut like FitMC",
...
public class AutoNomadHut extends Module
{
    ...
    public void onUpdate() {
        if (this.isDisabled() || AutoNomadHut.mc.player == null || KamiMod.MODULE_MANAGER.isModuleEnabled("Freecam")) {
            return;
        }
        if (this.offsetStep == 0) {
            ...
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] Starting Loop, current Player Slot: " + this.playerHotbarSlot);
            }
        ...
        for (int i = 0; i < (int)Math.floor(this.blockPerTick.getValue()); ++i) {
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] Loop iteration: " + this.offsetStep);
            }
    ...
    protected void onEnable() {
        ...
        if (this.debugMessages.getValue()) {
            MessageSendHelper.sendChatMessage("[AutoNomadHut] Enabling");
        }
        this.playerHotbarSlot = Wrapper.getPlayer().inventory.currentItem;
        this.lastHotbarSlot = -1;
        if (this.debugMessages.getValue()) {
            MessageSendHelper.sendChatMessage("[AutoNomadHut] Saving initial Slot  = " + this.playerHotbarSlot);
        }
    }
    ...
    protected void onDisable() {
        ...
        if (this.debugMessages.getValue()) {
            MessageSendHelper.sendChatMessage("[AutoNomadHut] Disabling");
        }
        if (this.lastHotbarSlot != this.playerHotbarSlot && this.playerHotbarSlot != -1) {
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] Setting Slot to  = " + this.playerHotbarSlot);
            }
            if (this.spoofHotbar.getValue()) {
    ...
    private void endLoop() {
        this.offsetStep = 0;
        if (this.debugMessages.getValue()) {
            MessageSendHelper.sendChatMessage("[AutoNomadHut] Ending Loop");
        }
        if (this.lastHotbarSlot != this.playerHotbarSlot && this.playerHotbarSlot != -1) {
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] Setting Slot back to  = " + this.playerHotbarSlot);
            }
            if (this.spoofHotbar.getValue()) {
                AutoNomadHut.mc.player.connection.sendPacket((Packet)new CPacketHeldItemChange(this.playerHotbarSlot));
            }
    ...
    private void placeBlock(final BlockPos blockPos) {
        if (!Wrapper.getWorld().getBlockState(blockPos).getMaterial().isReplaceable()) {
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] Block is already placed, skipping");
            }
            return;
        }
        if (!BlockInteractionHelper.checkForNeighbours(blockPos)) {
            if (this.debugMessages.getValue()) {
                MessageSendHelper.sendChatMessage("[AutoNomadHut] !checkForNeighbours(blockPos), disabling! ");
            }
            return;
        }
    ...
    public void placeBlockExecute(final BlockPos pos) {
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (!canBeClicked(neighbor)) {
                if (this.debugMessages.getValue()) {
                    MessageSendHelper.sendChatMessage("[AutoNomadHut] No neighbor to click at!");
                }
            }
            else {
                ...
                if (eyesPos.squareDistanceTo(hitVec) > 18.0625) {
                    if (this.debugMessages.getValue()) {
                        MessageSendHelper.sendChatMessage("[AutoNomadHut] Distance > 4.25 blocks!");
                    }
                }
                else {
                    ...
                    if (BlockInteractionHelper.blackList.contains(blockBelow) || BlockInteractionHelper.shulkerList.contains(blockBelow)) {
                        if (this.debugMessages.getValue()) {
                            MessageSendHelper.sendChatMessage("[AutoNomadHut] Sneak enabled!");
                        }
                        needSneak = true;
                    }
                    if (needSneak) {
                    ...
                    if (obiSlot == -1) {
                        if (this.debugMessages.getValue()) {
                            MessageSendHelper.sendChatMessage("[AutoNomadHut] No Obi in Hotbar, disabling!");
                        }
                        this.disable();
                        return;
                    }
                    if (this.lastHotbarSlot != obiSlot) {
                        if (this.debugMessages.getValue()) {
                            MessageSendHelper.sendChatMessage("[AutoNomadHut] Setting Slot to Obi at  = " + obiSlot);
                        }
                        if (this.spoofHotbar.getValue()) {
                    ...
                    if (needSneak) {
                        if (this.debugMessages.getValue()) {
                            MessageSendHelper.sendChatMessage("[AutoNomadHut] Sneak disabled!");
                        }
