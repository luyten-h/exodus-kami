...
@Module.Info(
        ...
        description = "Some random GUI feature put in here for no reason"
)
public class FakeClient extends Module {
    public Setting<Boolean> customizadfClient = register(Settings.booleanBuilder("Custom").withValue(false).build());
    ...
    private Setting<Integer> colorRed = register(Settings.integerBuilder("Red").withMinimum(1).withMaximum(255).withValue(0).build());
    private Setting<Integer> colorBlue = register(Settings.integerBuilder("Blue").withMinimum(1).withMaximum(255).withValue(0).build());
    private Setting<Integer> colorGreen = register(Settings.integerBuilder("Green").withMinimum(1).withMaximum(255).withValue(255).build());
    public Setting<String> customClientVAL = register(Settings.s("Custom Text","Baithck v1.0"));
    public int redForBG;
    ...
    private void checkSettingGuiColour(final Setting setting) {
        ...
        switch (s) {
            ...
            case "Green": {
                this.greenForBG = colorGreen.getValue();
                break;
            }
            case "Blue": {
                this.blueForBG = colorBlue.getValue();
                break;
            }
        }
    }
    ...
    public void onRender() {
        ...
        if (this.rainbow.getValue()) {
            if (this.customizadfClient.getValue()) {
                this.font.drawStringWithShadow(customClientVAL.getValueAsString(), cX, cY, rgb);
            } else {
                this.font.drawStringWithShadow("Baithck v1.0", cX, cY, rgb);
            }
        } else {
            if (this.customizadfClient.getValue()) {
                this.font.drawStringWithShadow(customClientVAL.getValueAsString(), cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            } else {
                this.font.drawStringWithShadow("Baithck v1.0", cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            }
        }
    }
}
