...
@Module.Info(
        ...
        description = "Shows the watermark of Exodus"
)
public class Watermark extends Module {
    private Setting<Boolean> version = register(Settings.booleanBuilder("Version").withValue(true).build());
    ...
    public void onRender() {
        ...
        if (this.rainbow.getValue()) {
            if (this.version.getValue()) {
                this.font.drawStringWithShadow("Exodus " + KamiMod.MODVERSMALL, cX, cY, rgb);
            } else {
                this.font.drawStringWithShadow("Exodus", cX, cY, rgb);
            }
        } else {
            if (this.version.getValue()) {
                this.font.drawStringWithShadow("Exodus " + KamiMod.MODVERSMALL, cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            } else {
                this.font.drawStringWithShadow("Exodus", cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            }
        }
    }
}
