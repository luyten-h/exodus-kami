...
@Module.Info(
        ...
        description = "A nice little greeting"
)
public class Welcome extends Module {
    private Setting<Boolean> addon = register(Settings.booleanBuilder("Nice Addon").withValue(true).build());
    ...
    public void onRender() {
        ...
        if (this.rainbow.getValue()) {
            if (this.addon.getValue()) {
                this.font.drawStringWithShadow("Welcome " + mc.getSession().getUsername() + " :^)", cX, cY, rgb);
            } else {
                this.font.drawStringWithShadow("Welcome " + mc.getSession().getUsername(), cX, cY, rgb);
            }
        } else {
            if (this.addon.getValue()) {
                this.font.drawStringWithShadow("Welcome " + mc.getSession().getUsername() + " :^)", cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            } else {
                this.font.drawStringWithShadow("Welcome " + mc.getSession().getUsername(), cX, cY, ColourUtils.toRGBA(this.redForBG, this.greenForBG, this.blueForBG, 255));
            }
        }
